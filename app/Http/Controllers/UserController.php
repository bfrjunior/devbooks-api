...
class UserController extends Controller
{
    ...
    public function follow($id)
    {
        ...
        if ($id == $this->loggedUser['id']) {
            $array['error'] = 'Você não pode seguir a sí mesmo.';
            return $array;
        }
        ...
        if ($userExists) {
            ...
            if ($relation) {
                ...
                $newRealtion = new UserRelation();
                $newRealtion->user_from = $this->loggedUser['id'];
                $newRealtion->user_to = $id;
                $newRealtion->save();
            }
        } else {
            $array['error'] = 'Usuário inexistente.';
    ...
    public function followers($id)
    {
        ...
        $userExists = User::find($id);
        if ($userExists) {
            $follwers = UserRelation::where('user_to', $id)->get();
            $following = UserRelation::where('user_from', $id)->get();

            $array['followers'] = [];
            $array['following'] = [];

            foreach ($follwers as $item) {
